...
use grip::analysis::struct_registry::{StructRegistry, field_type_head};
use std::path::PathBuf;
use syn::Type;
use syn::parse_str;

#[test]
fn concrete_fields_of_a_struct_returns_each_field_against_its_type_head() {
    // Arrange
    let source = String::from("pub struct Holder { pub inner: Inner, pub count: u32 }");
    let registry = StructRegistry::build(&[(PathBuf::from("lib.rs"), source)]);

    // Act
    let fields = registry.concrete_fields_of("Holder");

    // Assert
    assert_eq!(fields.get("inner").map(String::as_str), Some("Inner"));
    assert_eq!(fields.get("count").map(String::as_str), Some("u32"));
}

#[test]
fn concrete_fields_of_an_unknown_type_returns_an_empty_map() {
    // Arrange
    let registry = StructRegistry::build(&[]);

    // Act
    let fields = registry.concrete_fields_of("NoSuchStruct");
...
#[test]
fn field_type_head_on_a_path_type_returns_its_last_segment() {
    // Arrange
    let ty: Type = parse_str("std::collections::HashMap").expect("parse type");

    // Act
    let head = field_type_head(&ty);
...
#[test]
fn field_type_head_on_a_type_that_is_not_a_path_returns_an_empty_string() {
    // Arrange
    let ty: Type = parse_str("[u8; 4]").expect("parse type");
