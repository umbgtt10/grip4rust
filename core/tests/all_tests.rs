// Copyright 2026 Umberto Gotti <umberto.gotti@umbertogotti.dev>
// Licensed under the MIT License
// SPDX-License-Identifier: MIT

mod app_tests;

mod args_tests;

mod cache_tests;

mod collector_tests;

mod config_tests;

mod contribution_schedule_tests;

mod default_scorer_tests;

mod dep_weight_ladder_tests;

mod fs_walk_tests;

mod function_info_tests;

mod function_purity_tests;

mod grip_report_tests;

mod hidden_dep_finder_tests;

mod io_call_finder_tests;

mod item_classifier_tests;

mod item_counts_tests;

mod lib_tests;

mod main_tests;

mod method_purity_registry_tests;

mod module_stats_tests;

mod no_op_cache_store_tests;

mod offender_tests;

mod offenders_renderer_tests;

mod overall_stats_tests;

mod overall_summary_renderer_tests;

mod stdout_reporter_tests;

mod struct_registry_tests;

mod unsafe_finder_tests;

mod verbose_functions_renderer_tests;
