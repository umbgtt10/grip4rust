...
use grip::item_classifier::ItemClassifier;
use grip::visibility_level::VisibilityLevel;
use syn::{Attribute, ImplItemFn, ItemFn, Path as SynPath};

fn item_fn(source: &str) -> ItemFn {
    syn::parse_str(source).expect("function should parse")
}

fn impl_method(source: &str) -> ImplItemFn {
    syn::parse_str(source).expect("method should parse")
}
...
fn attrs(source: &str) -> Vec<Attribute> {
    let parsed: ItemFn = syn::parse_str(&format!("{source} fn placeholder() -> u32 {{ 1 }}"))
...
fn classify_visibility_for_pub_returns_pub() {
    // Arrange
    let parsed: ItemFn = syn::parse_str("pub fn f() -> u32 { 1 }").expect("parses");
...
fn classify_visibility_for_pub_crate_returns_pub_crate() {
    // Arrange
    let parsed: ItemFn = syn::parse_str("pub(crate) fn f() -> u32 { 1 }").expect("parses");
...
fn classify_visibility_for_inherited_returns_private() {
    // Arrange
    let parsed: ItemFn = syn::parse_str("fn f() -> u32 { 1 }").expect("parses");
